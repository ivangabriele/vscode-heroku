...
// https://gist.github.com/arendjr/415747652a8c79f12b005ce3cfb2f808
  ...
  factory: require => {
    ...
    class ListCommand extends BaseCommand {
      ...
      async execute() {
        ...
        const { dependencies: packageJsonDependencies } = JSON.parse(packageJsonContents)
        ...
        function addDependency(packageName, versionRange) {
          const packageInfo = lookup(resolved, getLockFileKey(packageName, versionRange))
          if (!packageInfo) {
            throw new Error(`Cannot resolve "${packageName}" with version range "${versionRange}"`)
          }
          ...
          if (dependencies) {
            const children = Object.entries(dependencies).map(([name, range]) => ({ name: `${name}@${range}` }))
